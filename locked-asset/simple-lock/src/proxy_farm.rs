    ...
    + multiversx_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn enter_farm_locked_token(
        ...
        let proxy_lp_payment: EsdtTokenPayment<Self::Api> = payments.get(0).as_refs().to_owned_payment();
    ...
    fn exit_farm_locked_token(&self) -> ExitFarmThroughProxyResultType<Self::Api> {
        let payment: EsdtTokenPayment<Self::Api> =
            self.call_value().single_esdt().as_refs().to_owned_payment();
